...
public class MatchRepository : IMatchRepository
{
    ...
    {
        return _context
            .Matches.Where(m => m.MatchFinished == matchFinished)
            .OrderBy(m => m.MatchId)
            .Select(m => new MatchDTOResponse
            {
                ...
                MatchTeamAOdds = Math.Round(
                        (m.MatchTeamAValue + m.MatchTeamBValue) / m.MatchTeamAValue,
                        2
                    )
                    .ToString("###.##"),
                MatchTeamBOdds = Math.Round(
                        (m.MatchTeamAValue + m.MatchTeamBValue) / m.MatchTeamBValue,
                        2
                    )
                    .ToString("###.##"),
